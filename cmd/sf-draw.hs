...
vec_normalise :: (Fractional b, Ord b, V.Storable b) => Bool -> V.Vector b -> V.Vector b
vec_normalise hlf = if hlf then vec_normalise_2 else vec_normalise_1
...
sf_draw :: (Bool,Bool,Bool,Bool) -> Int -> Int -> FilePath -> FilePath -> IO ()
sf_draw (nrm,hlf,inv,sym) h ch sf_fn pbm_fn = do
  (hdr,Just vec') <- SF.read_vec sf_fn
  let vec = ((if sym then V.map abs else id) .
             (if inv then id else V.map negate) .
             (if nrm then vec_normalise hlf else id)) vec'
...
main = do
  a <- getArgs
  case a of
    ["pbm",nrm,hlf,inv,sym,h,ch,sf_fn,pbm_fn] ->
        sf_draw (nrm == "t",hlf == "t",inv == "t",sym == "t") (read h) (read ch) sf_fn pbm_fn
