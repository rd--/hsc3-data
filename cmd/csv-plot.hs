...
csv_plot_1 :: Int -> FilePath -> IO ()
csv_plot_1 i = with_csv_col (\c -> plotTable1 (c !! i))

csv_plot_2 :: (Int,Int) -> FilePath -> IO ()
csv_plot_2 (i,j) = with_csv_col (\c -> plot_p2_ln [zip (c !! i) (c !! j)])

csv_plot_3 :: (Int,Int,Int) -> FilePath -> IO ()
csv_plot_3 (i,j,k) = with_csv_col (\c -> plot_p3_ln [zip3 (c !! i) (c !! j) (c !! k)])

main :: IO ()
main = do
  a <- getArgs
  case a of
    ["ln",fn,x] -> csv_plot_1 (read x) fn
    ["ln",fn,x,y] -> csv_plot_2 (read x,read y) fn
    ["ln",fn,x,y,z] -> csv_plot_3 (read x,read y,read z) fn
    _ -> putStrLn "csv-plot ln csv-file x [y z]"
