...
svl_get_model :: Xml.Element -> Xml.Element
svl_get_model = Xml.x_get_elem_path ["data", "model"]

-- | Get model type and sub-type.
svl_model_type :: Xml.Element -> (String, Maybe String)
svl_model_type m =
  if Xml.x_elem_name m /= "model"
    then error "svl_model_ty"
    else (Xml.x_get_attr "type" m, Xml.findAttr (Xml.unqual "subtype") m)

-- | Get /model/ and ensure it has the indicated type and sub-type.
svl_get_model_of_type :: (String, Maybe String) -> Xml.Element -> Xml.Element
...
svl_get_dataset :: Xml.Element -> Xml.Element
svl_get_dataset = Xml.x_get_elem_path ["data", "dataset"]

-- | Load the /sv/ element from an Svl file.
svl_load :: FilePath -> IO (Maybe Xml.Element)
svl_load = Xml.xml_load

svl_load_err :: FilePath -> IO Xml.Element
...
svl_parse_sparse :: Maybe String -> (Xml.Element -> Svl_Pt t) -> Xml.Element -> (Sr, Seq.Wseq Frame t)
...
svl_load_sparse_note :: FilePath -> IO (Sr, Seq.Wseq Frame Svl_Note)
svl_load_sparse_note = fmap (svl_parse_sparse (Just "note") svl_parse_point_d3) . svl_load_err

svl_load_sparse_time_instant :: FilePath -> IO (Sr, Seq.Wseq Frame ())
...
svl_wseq_to_tm :: (Sr -> Frame -> t) -> (Sr, Seq.Wseq Frame u) -> Seq.Wseq t u
svl_wseq_to_tm tm_f (sr, sq) =
  let bimap1 f (t, u) = (f t, f u)
  in Seq.wseq_tmap (bimap1 (tm_f sr)) sq

svl_load_sparse_note_tm :: (Sr -> Frame -> t) -> FilePath -> IO (Seq.Wseq t Svl_Note)
svl_load_sparse_note_tm tm_f fn = fmap (svl_wseq_to_tm tm_f) (svl_load_sparse_note fn)

svl_load_sparse_note_csec :: Int -> FilePath -> IO (Seq.Wseq T.Csec Svl_Note)
...
svl_load_sparse_note_sec :: FilePath -> IO (Seq.Wseq T.Sec Svl_Note)
svl_load_sparse_note_sec = svl_load_sparse_note_tm svl_frame_to_sec

svl_load_sparse_note_mnn_accum :: (Ord t, Num t) => (Sr -> Frame -> t) -> FilePath -> IO (Bool, Seq.Tseq t ([Pitch.Midi], [Pitch.Midi], [Pitch.Midi]))
svl_load_sparse_note_mnn_accum tm_f =
  fmap (Seq.wseq_begin_end_accum . Seq.wseq_map (\(e, _, _) -> e))
...
type Svl_Node_m t = Svl_Node t Pitch.Midi

svl_load_node_m :: Ord t => (Sr -> Frame -> t) -> (Pitch.Midi -> Pitch.Midi) -> FilePath -> IO [Svl_Node_m t]
svl_load_node_m tm_f mnn_f fn = do
  pt <- svl_load_sparse_note_tm tm_f fn
  let n = List.collate (map (\((tm, du), (mnn, _, _)) -> (tm, (mnn, du))) pt)
      to_p (tm, (mnn, du)) = (tm, (map mnn_f mnn, du))
  return (map (to_p . Data.Bifunctor.second unzip) n)

-- * Svl_Node_p

type Svl_Node_p t = Svl_Node t Pitch.Pitch

svl_load_node_p :: Ord t => (Sr -> Frame -> t) -> (Pitch.Midi -> Pitch.Midi) -> FilePath -> IO [Svl_Node_p t]
...
svl_node_p_csec_pp (tm, (p, du)) =
  ...
  in unwords
      [ csec_tm_pp tm
      , Data.List.intercalate "," (map Pitch.pitch_pp p)
      , Data.List.intercalate "," (map csec_du_pp du)
      ]
