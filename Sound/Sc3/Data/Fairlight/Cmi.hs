...
import qualified Music.Theory.Directory.Find as Directory.Find {- hmt-base -}
...
-}
type Cmi_Sfz = (FilePath, FilePath, Double, Pitch.Midi, Char, (Math.U32, Math.U32), (Double, Double))

-- | Parse Sfz <region>.
cmi_sfz_rgn_parse :: FilePath -> Sfz.Sfz_Region -> Cmi_Sfz
cmi_sfz_rgn_parse dir r =
  let vol = Sfz.sfz_region_volume r
      (mnn, _, _) = Sfz.sfz_region_key r
      (lm, lp) = Sfz.sfz_region_loop_data r
  in ( dir
     , Sfz.sfz_region_sample r
     , vol
     , mnn
     , Sfz.sfz_loop_mode_sym lm
     , Data.Maybe.fromMaybe (0, 0) lp -- allow NON-Cmi files...
     , (Sfz.sfz_region_ampeg_attack r, Sfz.sfz_region_ampeg_release r)
     )

-- | Load and parse Cmi Sfz.
cmi_load_sfz :: FilePath -> IO Cmi_Sfz
cmi_load_sfz fn = do
  (_, _, rgn) <- Sfz.sfz_load_data fn
  case rgn of
    [r] -> return (cmi_sfz_rgn_parse (System.FilePath.takeDirectory fn) r)
    _ -> error "cmi_load_sfz?"

-- | Pretty-printer.
cmi_sfz_pp :: (String, Cmi_Sfz) -> String
cmi_sfz_pp (nm, (_, _, _, _, l1, (l2, l3), (e1, e2))) =
  Text.Printf.printf "%-24s %c %5d %5d %3.1f %3.1f" nm l1 l2 l3 e1 e2
...
cmi_load_dir dir = do
  fn <- Directory.Find.dir_find_ext_rel ".sfz" dir
  let nm_seq = map System.FilePath.dropExtension (Data.List.sort fn)
  cmi_seq <- mapM (cmi_load_sfz . (dir <>)) (Data.List.sort fn)
