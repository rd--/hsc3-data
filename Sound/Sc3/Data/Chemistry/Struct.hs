...
struct_calculate_bonds tol (nm, (n_a, _), dsc, a, _b) =
  let b = map fst (Elements.calculate_bonds sym_radius tol a)
      n_b = length b
...
struct_stat s =
  let (nm, (n_a, n_b), dsc, a, _) = s
      ...
      q =
        ...
        , ("Elements", unwords u)
        , ("Formula", Elements.hill_formula e)
...
struct_pp k (nm, (n_a, n_b), dsc, a, b) =
  let hdr =
      ...
      a_pp (e, (x, y, z)) = unwords (e : map (Show.double_pp k) [x, y, z])
...
-- | 'Struct' to 'Graph.Lbl' with 'Atom' labels at vertices.
struct_to_lbl :: Struct -> Graph.Lbl_ Atom
struct_to_lbl (_, _, _, a, b) = (zip [0 ..] a, zip b (repeat ()))

-- | 'Graph.v3_graph_to_obj' of 'struct_to_lbl', element names are discarded.
