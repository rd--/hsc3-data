...
store_preset :: U8 -> SYSEX
store_preset mm = [0xF0,0x00,0x20,0x6B,0x7F,0x42,0x06,mm,0xF7]

recall_preset :: U8 -> SYSEX
recall_preset mm = [0xF0,0x00,0x20,0x6B,0x7F,0x42,0x05,mm,0xF7]

-- * ENC = encoder, cm = CC-mode

{- | Encoder control-change mode.

k = control-id (ie. 0x20-0x2F),
...
pad_nm_set :: U8 -> (U8, U8, U8) -> [SYSEX]
pad_nm_set k (ch,mnn,md) =
  [set_sysex 0x01 k 0x09 -- midi-note-number mode
  ,set_sysex 0x02 k ch
...
pad_cm_set k (ch,cc,(off,on),md) =
  [set_sysex 0x01 k 0x08 -- control-change mode
  ,set_sysex 0x02 k ch
  ,set_sysex 0x03 k cc
